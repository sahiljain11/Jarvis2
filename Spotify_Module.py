...
class SpotipyModule:
    ...
    def play_music_from_playlist(self):
        ...
        search_results = ((self.generate_token().playlist_tracks(self.find_a_playlist(str(input("Enter Playlist: "))),fields='items,uri,name'))['items'])
    ...
    def add_song_to_queue(self,song_title):
        ...
        for songz in range(0,len(search_results['tracks']['items'])):
            ...
            song_choices_for_queue[song_index] = str(song_number) +') ' + search_results['tracks']['items'][songz]['name']
            song_number += 1
            song_index += 1
    ...
    def find_a_playlist(self,playlist_title):
        ...
        '''
                if search_results is None or len(playlist_to_be_queued) == 0:
            editted_search = ''
            for word in playlist_title.split():
                editted_search += self.spellchecker.correction(word) + ' '

        '''

        for songz in range(0, len(search_results['playlists']['items'])):
            playlist_to_be_queued[playlist_index] = str(playlist_number) + ') ' + search_results['playlists']['items'][songz]['name']
...
class QueueDataStructure:
    ...
    def push(self, new_data):
        ...
        if self.number_of_nodes == 0:
            ...
            self.number_of_nodes += 1
        '''
        print(self.pointer.value,'pointer value')
        print(self.pointer.get_prev().value, 'prev pointer')

        print(new_node.get_next().value,'next')
        print(new_node.get_next().get_next().value, ' next value')
        print(new_node.get_prev().value,'prev')
        print('\n')
        '''
        return
...
spy = SpotipyModule(environ.get('USER'),environ.get('CLIENT_ID'),environ.get('CLIENT_SECRET'),'https://Jarvis2_Spotify_Mod_HackAThon.com')

spy.play_music_from_playlist()